# utils.py (lean version for API only)
...
def get_best_name_matches(search_name, candidates, limit=50, threshold=80):
    """Robust fuzzy match with some basic token heuristics."""
    def preprocess(name):
...
def refresh_opensanctions_data():
    """
    Download latest OpenSanctions CSVs (HMT sanctions + PEPs), tag source_type,
    combine, write to a single parquet for fast search.
    """
    urls = {
...
def perform_opensanctions_check(name, dob, entity_type="Person", parquet_path="data/opensanctions.parquet"):
    ...
    def match_priority(idx):
        row = df.iloc[idx]
        candidate_name = normalize_text(str(row.get("name", "")))
    ...
    result = {
        ...
        "Regime": top_match.get("regime"),            # may be NaN depending on dataset
...
def load_ofsi_data(file_path):
    ...
    df["Name"] = df["Raw Name"].apply(lambda n: " ".join(n.split()[:1] + n.split()[-1:])).apply(normalize_text)
...
def query_wikidata(name):
    # very light PEP hint; safe to leave as-is or remove if not needed
    q = f'''
